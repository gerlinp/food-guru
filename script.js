      ...
      <div className="dt-translator-right">
        {phase==='config'&&(
          <div className="fadeInUp" style={{display:'flex',flexDirection:'column',gap:18}}>
            <div>
              <div style={{display:'flex',alignItems:'center',gap:8,color:'var(--orange)'}}><Icon.sparkle/><span className="dt-eyebrow" style={{color:'var(--orange)'}}>Chef Tool</span></div>
            ...
            <div className="dt-translator-section">
              <h4>Servings</h4><div className="sub">How many people are eating?</div>
              <div style={{display:'flex',alignItems:'center',gap:16}}>
                ...
                <span style={{fontSize:13,color:'var(--ink-mute)'}}>{servings>recipe.servings?`↑ ${servings-recipe.servings} more than original`:servings<recipe.servings?`↓ ${recipe.servings-servings} fewer than original`:'matches original'}</span>
              </div>
            </div>
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:18}}>
              <div className="dt-translator-section">
                <h4>Dietary style</h4><div className="sub">Pick any that apply</div>
                <div className="dt-chip-grid">{DT_DIETARY.map(item=><button key={item} className={`dt-chip ${diet.has(item)?'active':''}`} onClick={()=>tog(diet,setDiet,item)}>{diet.has(item)&&<span style={{marginLeft:-2}}>✓</span>}{item}</button>)}</div>
              </div>
              <div className="dt-translator-section">
                <h4>Allergies & avoid</h4><div className="sub">Routed around in every swap</div>
                <div className="dt-chip-grid">{DT_ALLERGIES.map(item=><button key={item} className={`dt-chip ${allergies.has(item)?'active':''}`} onClick={()=>tog(allergies,setAllergies,item)}>{allergies.has(item)&&<span style={{marginLeft:-2}}>✓</span>}{item}</button>)}</div>
              </div>
            </div>
            <div className="dt-translator-section">
              <h4>Cuisine remix</h4><div className="sub">Reinterpret the dish through another tradition</div>
              <div className="dt-chip-grid">{DT_CUISINES.map(c=><button key={c} className={`dt-chip ${cuisine===c?'active':''}`} onClick={()=>setCuisine(c)}>{c}</button>)}</div>
            </div>
            <div style={{display:'grid',gridTemplateColumns:'1.4fr 1fr',gap:18}}>
              <div className="dt-translator-section">
                <h4>Time cap</h4><div className="sub">Total time including prep</div>
                ...
                <div style={{display:'flex',justifyContent:'space-between',fontSize:11,color:'var(--ink-mute)',marginTop:4}}><span>15m</span><span>60m</span><span>120m</span><span>180m</span></div>
              </div>
              <div className="dt-translator-section">
                <h4>Skill level</h4><div className="sub">How techy can it get?</div>
                <div className="dt-segmented">{DT_SKILLS.map(s=><button key={s} className={skill===s?'active':''} onClick={()=>setSkill(s)}>{s}</button>)}</div>
              </div>
            </div>
            <div className="dt-translator-section">
              <h4>Kitchen constraints</h4><div className="sub">Anything you're working without?</div>
              <div className="dt-chip-grid">{DT_EQUIPMENT.map(item=><button key={item} className={`dt-chip ${equipment.has(item)?'active':''}`} onClick={()=>tog(equipment,setEquipment,item)}>{equipment.has(item)&&<span style={{marginLeft:-2}}>✓</span>}{item}</button>)}</div>
            </div>
            <div className="dt-translator-section">
              <h4>Pantry notes</h4><div className="sub">What you have, what's missing, what you want to use up</div>
              <textarea className="dt-input" style={{minHeight:90}} placeholder="e.g. I have arborio rice but no tagliatelle, half a lemon, and some leftover white miso..." value={pantry} onChange={e=>setPantry(e.target.value)}/>
            </div>
            <div className="dt-sticky-cta">
              <button onClick={onTranslate} className="dt-btn primary lg" style={{flex:1}}><Icon.sparkle/> Reshape recipe</button>
            </div>
          </div>
        )}
  ...
  return (
    <div className="fadeInUp" style={{height:'100%',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',gap:36,padding:40}}>
      <div style={{position:'relative',width:120,height:120}}>
        ...
        <div style={{position:'absolute',inset:0,display:'flex',alignItems:'center',justifyContent:'center',color:'var(--orange)'}}><Icon.sparkle style={{width:44,height:44}}/></div>
      </div>
  ...
  return (
    <div className="fadeInUp" style={{display:'flex',flexDirection:'column',gap:22}}>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start'}}>
        <div>
          <div style={{display:'flex',alignItems:'center',gap:8,color:'var(--orange)'}}><Icon.sparkle/><span className="dt-eyebrow" style={{color:'var(--orange)'}}>Reshaped</span></div>
      ...
      <div style={{background:'var(--navy)',color:'var(--cream)',borderRadius:22,padding:26}}>
        <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:10}}><Icon.sparkle style={{color:'var(--orange)'}}/><span className="dt-eyebrow" style={{color:'var(--orange)'}}>Guru's notes</span></div>
        <p style={{margin:0,fontSize:15,lineHeight:1.6,color:'rgba(250,245,236,0.88)'}}>Kept the brown-butter soul of the dish, swapped the dairy and gluten, scaled to {servings}, and trimmed it into a single skillet flow that fits in {time} minutes.</p>
      </div>
      <div>
      ...
      <div>
        ...
        {translated.steps.map((s,i)=>{
          ...
          return <div key={i} className="dt-step"><div className="dt-step-num">{String(i+1).padStart(2,'0')}</div><p>{parts}</p></div>;
        })}
      </div>
      <div className="dt-sticky-cta">
